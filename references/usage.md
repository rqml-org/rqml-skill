...
- `rqml skeleton req` emits a schema-valid `<req>` to fill in (also `edge`, `testCase`, `stateMachine`). Never invent element shapes.
...
- `rqml validate` after every edit; never leave the spec invalid.
...
- `rqml check` must exit 0 at the project's strictness — it combines validation, coverage, and drift. `rqml lint`, `rqml status`, and `rqml matrix` surface quality and coverage detail.
